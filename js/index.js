...
$(document).ready(function(){
	...
	$rotaryArrow.click(function(){
		...
		if($.cookie('rotary_time_hzh')==null){
			...
			//再在此生成IP-cookie
			var $time = new Date();
			var $_COOKIE = $.cookie('rotary_time_hzh',$time,{ expires:24 , path: "/"});
	...
	$("form").submit(function(){
        ...
        if(tel=="" || name==""){
            alert('属性不能为空');
            return false;
        }else if (tel.search(/^(((1[3578]{1}[0-9]{1})|(15[0-9]{1}))+\d{8})$/)== -1){ 
                alert("手机号码有误，请重新输入");  
                return false; 
	...
	$looper_close.click(function(){
		$raotary_looper.css({"background-image":'url(images/rotary_min.png)','opacity':"1",'background-color':'rgba(0,0,0,0)','background-size':'100% auto','background-repeat':'no-repeat','background-position':'center center'});
